...
from config import LLM_RESPONSE_PATH_OF_INTEREST, AGREEMENTS_PATH, FRONTEND_DOCS_PATH
...
def create_markdown(input_path, output_path):
    ...
    llm_path = LLM_RESPONSE_PATH_OF_INTEREST + file_name + '/clean_responses.json'
...
if __name__ == '__main__':
    ...
    for section in structure:
        ...
        if '.txt' in section_clean:
            create_markdown(AGREEMENTS_PATH+structure[section], OUTPUT_FOLDER + section_clean)
            continue
        ...
        # repeat process for subsections
            for subsection in structure[section]:
                ...
                if '.txt' in subsection_clean:
                    create_markdown(AGREEMENTS_PATH+structure[section][subsection], OUTPUT_FOLDER + section_clean + '/' + subsection_clean)
                    continue
                ...
                for subsubsection in structure[section][subsection]:
                    ...
                    if '.txt' in subsubsection_clean:
                        create_markdown(AGREEMENTS_PATH+structure[section][subsection][subsubsection], OUTPUT_FOLDER + section_clean + '/' + subsection_clean + '/' + subsubsection_clean)
                        continue
                    ...
                    for subsubsubsection in structure[section][subsection][subsubsection]:
                        ...
                        if '.txt' in subsubsubsection_clean:
                            create_markdown(AGREEMENTS_PATH+structure[section][subsection][subsubsection][subsubsubsection], OUTPUT_FOLDER + section_clean + '/' + subsection_clean + '/' + subsubsection_clean + '/' + subsubsubsection_clean)
                            continue
